...
def _linux_scan_roots():
    roots = [Path.home()]
    home_parent = Path.home().parent if Path.home().name != "root" else Path("/home")
...
def _default_paths():
    return _expand_scan_paths([str(path) for path in _linux_scan_roots()])
...
class AegisDlpService:
    ...
    def submit_threats(self, server_url, token, agent_id, incidents):
        if not server_url or not incidents:
            return None
        max_upload = int(self.cfg.get(self.module_name, {}).get("max_upload_size_mb", 5) or 5)
